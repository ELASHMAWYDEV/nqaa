...
class statsController extends Controller
{
    ...
    public function getPaymentsStats($period = 'today, now')
    {
        //To select the period with js
        ...
        //Explode the period to array
        $period = explode(", ", $period);

        //Get the stats
        foreach ($payments as $payment) {
            if (strtotime($payment->create_date) > strtotime($period[0]) && strtotime($payment->create_date) < strtotime($period[1])) {
